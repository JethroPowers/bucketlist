...
class BucketlistTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        #     self.assertEqual(results[0]['name'],self.bucketlist['name'] )
        ...
        #
        # def test_completed_or_not(self):
        #     """Test API can change the completion_status."""
        #     rv = self.client().post('/bucketlists/', data={'name': 'pick up water'})
        #     print(json.loads(rv.data))
        #     self.assertEqual(rv.status_code, 201)
        #     rv = self.client().put('/bucketlists/1', data={'completion_status': 'true'})
        #     self.assertEqual(rv.status_code, 200)
        #     results = self.client().get('/bucketlists/1')
        #     print(json.loads(results.data))
        #     bucketlist = json.loads(results.data)
        #     self.assertEqual(True, bucketlist['completion_status'])
        #
        # def test_create_nameless_bucketlist_fails(self):
        #     res = self.client().post('/bucketlists/', data={'completion_status': 'true'})
        #     self.assertEqual(res.status_code, 400)
        #     response = json.loads(res.data)
        #     self.assertEqual('the json data is missing a key name', response['message'])
        #     self.assertEqual('error', response['status'])
        #
        # def test_bucketlist_already_exists(self):
        #     res = self.client().post('/bucketlists/', data=self.bucketlist)
        #     self.assertEqual(res.status_code, 201)
        #     rv = self.client().post('/bucketlists/', data=self.bucketlist)
        #     self.assertEqual(rv.status_code, 409)
        #     response = json.loads(rv.data)
        #     self.assertEqual('the name has been used before', response['message'])
        #     self.assertEqual('error', response['status'])
    ...
    def test_bucketlist_creation(self):
        ...
        print(json.loads(result.data.decode()))
